...
int  main(int argc,char*argv[]) {
		notice("文件加密器 版本:v1.4 date:2020\\2\\19 by znkzz");
	if(argc<5) {
		error("命令行参数错误!");
		exit(-1);
	}
...
if (argc==6&&strcmp(argv[5],"-del")==0&&code==0) {
	remove(argv[3]);
}else if(argc==6&&strcmp(argv[5],"-del")==0&&code!=0)
{
